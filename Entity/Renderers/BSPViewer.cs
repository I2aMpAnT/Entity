...
#pragma warning restore CS0649
        ...
        /// <summary>
        /// Helper class for collecting GLB primitive data.
        /// </summary>
        private class GLBPrimitiveData
        {
            public List<float> Positions = new List<float>();
        ...
        /// <summary>
        /// Exports the current map to a 3D model file (GLB format with raw coordinates).
        /// </summary>
        private void ExportMapTo3D()
        {
            if (bsp == null)
            ...
            using (SaveFileDialog sfd = new SaveFileDialog())
            {
                ...
                sfd.Title = "Export 3D Model (Raw Coordinates)";
                sfd.FileName = bsp.Name ?? "map";
                ...
                if (sfd.ShowDialog() == DialogResult.OK)
                {
                    try
                    {
                        ...
                        MessageBox.Show($"Model exported successfully to:\n{sfd.FileName}\n\nCoordinates are raw Halo 2 units.", "Export Complete", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    }
                    catch (Exception ex)
                    {
                        MessageBox.Show($"Export failed: {ex.Message}", "Export Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
        ...
        /// <summary>
        /// Exports all maps in the maps folder to GLB files.
        /// </summary>
        private void ExportAllMapsToGLB()
        {
            if (render?.device == null)
            ...
            // Get all map files including subdirectories (for DLC maps)
            string[] mapFiles = Directory.GetFiles(mapsFolder, "*.map", SearchOption.AllDirectories);
            if (mapFiles.Length == 0)
            {
                MessageBox.Show("No .map files found.", "Export Error", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                return;
            }
        ...
        /// <summary>
        /// Exports the current BSP to GLB format with raw coordinates (no transformation).
        /// Coordinates match Halo 2 world units for player position overlay.
        /// </summary>
        private void ExportToGLB(string filePath)
        {
                            ...
                            foreach (var face in faces)
                            {
                                for (int fi = 0; fi < 3; fi++)
                                {
                                    ...
                                    if (!vertexMap.ContainsKey(vi))
                                    {
                                        ...
                                        // RAW COORDINATES - no transformation, preserves Halo 2 coordinate system
                                        prim.Positions.Add(v.X);
                                        prim.Positions.Add(v.Y);
                                        prim.Positions.Add(v.Z);
            ...
            using (BinaryWriter binWriter = new BinaryWriter(binStream))
            {
                ...
                List<(int bufferView, int componentType, int count, string type, float[] min, float[] max)> accessors =
                    new List<(int, int, int, string, float[], float[])>();
                List<(int posAccessor, int uvAccessor, int idxAccessor, int material)> primitiveInfo =
                    new List<(int, int, int, int)>();
                ...
                foreach (int shaderIdx in usedShaderIndices)
                {
                    shaderToMaterial[shaderIdx] = matIdx++;
                }

                foreach (int shaderIdx in usedShaderIndices)
                {
                    var prim = primitivesByShader[shaderIdx];
                    if (prim.Positions.Count == 0) continue;
                ...
                foreach (int shaderIdx in usedShaderIndices)
                {
                    if (bsp.Shaders?.Shader != null && shaderIdx < bsp.Shaders.Shader.Length)
                    {
                        var shader = bsp.Shaders.Shader[shaderIdx];
                        Bitmap textureBitmap = null;
                        ...
                        if (textureBitmap != null)
                        {
                            ...
                            {
                                textureBitmap.Save(pngStream, System.Drawing.Imaging.ImageFormat.Png);
                                byte[] pngData = pngStream.ToArray();

                                int imgOffset = (int)binStream.Position;
                                binWriter.Write(pngData);
                                while (binStream.Position % 4 != 0) binWriter.Write((byte)0);
                                int imgLength = pngData.Length;
                ...
                for (int i = 0; i < primitiveInfo.Count; i++)
                {
                    var pi = primitiveInfo[i];
                    json.AppendFormat("      {{ \"attributes\": {{ \"POSITION\": {0}, \"TEXCOORD_0\": {1} }}, \"indices\": {2}, \"material\": {3} }}{4}\n",
                        pi.posAccessor, pi.uvAccessor, pi.idxAccessor, pi.material,
                        i < primitiveInfo.Count - 1 ? "," : "");
                }
                json.Append("    ]\n");
                json.Append("  }],\n");
                ...
                for (int i = 0; i < usedShaderIndices.Count; i++)
                {
                    int shaderIdx = usedShaderIndices[i];
                    json.Append("    {\n");
                    json.AppendFormat("      \"name\": \"material_{0}\",\n", shaderIdx);
                    json.Append("      \"pbrMetallicRoughness\": {\n");
                    if (textureImageIndex.ContainsKey(shaderIdx))
                    {
                        json.AppendFormat("        \"baseColorTexture\": {{ \"index\": {0} }},\n", textureImageIndex[shaderIdx]);
                ...
                if (textureImageIndex.Count > 0)
                {
                    ...
                    for (int i = 0; i < imageBufferViews.Count; i++)
                    {
                        json.AppendFormat("    {{ \"bufferView\": {0}, \"mimeType\": \"image/png\" }}{1}\n",
                            bufferViews.Count + i, i < imageBufferViews.Count - 1 ? "," : "");
                    }
                    json.Append("  ],\n");
                }
                ...
                for (int i = 0; i < accessors.Count; i++)
                {
                    ...
                    if (a.min != null && a.max != null)
                    {
                        // Use G9 format for full float precision
                        json.AppendFormat(System.Globalization.CultureInfo.InvariantCulture,
                            ", \"min\": [{0:G9}, {1:G9}, {2:G9}], \"max\": [{3:G9}, {4:G9}, {5:G9}]",
                            a.min[0], a.min[1], a.min[2], a.max[0], a.max[1], a.max[2]);
                    }
